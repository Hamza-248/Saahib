document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderTrendingPosts() {
    ...
    trendingGrid.innerHTML = trendingPosts
      .map((post, index) => createPostCard(post, "trending-card", index))
      .join("");
  ...
  function renderFilterControls() {
    const categories = [
      "All",
      ...new Set(nonFeaturedPosts.map((p) => p.category)),
    ];
    filterScrollContainer.innerHTML = categories
      .map(
        (cat) =>
  ...
  function renderBlogGrid(append = false) {
    ...
    viewMoreContainer.classList.toggle(
      "d-none",
  ...
  function setupFilterScrolling() {
    ...
    function updateArrowVisibility() {
      ...
      const atEnd =
        filterScrollContainer.scrollLeft + filterScrollContainer.clientWidth >=
        filterScrollContainer.scrollWidth - 10;
