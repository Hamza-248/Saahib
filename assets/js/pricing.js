document.addEventListener("DOMContentLoaded", async function () {
  ...
  function handleLevelSelection(levelKey) {
    ...
    levelToggle.dataset.value = levelKey;
    levelToggle.querySelector("span").textContent =
  ...
  function renderCardHTML(level, courseKey, colClass = "") {
    ...
    const enrollLink = plan.note
      ? "https://wa.me/923266855525"
                    ...
                    <div class="${colClass}">
                        <div class="${wrapperClass}">
                            <div class="pricing-card" data-level="${level}" data-course="${courseKey}">
                                ...
                                ${
                                  isCustomizable
                                    ? `<button class="btn btn-customize" onclick="openSubjectModal('${level}', '${courseKey}')">Customize Subjects</button>`
                                    : ""
                                }
  ...
  function renderAllCardsForLevel(levelKey) {
    ...
    for (const courseKey in courses) {
      resultContainer.innerHTML += renderCardHTML(
        levelKey,
        courseKey,
        colClass
      );
  ...
  window.openSubjectModal = (level, courseKey) => {
    ...
    plan.available_subjects.forEach((subject, index) => {
      ...
      subjectGrid.innerHTML += `
        ...
        isChecked ? "checked" : ""
      } onchange="updateDynamicPricing('${level}', '${courseKey}')">
  ...
  window.updateDynamicPricing = (level, courseKey) => {
    ...
    const paymentTerm = plan.payment_type
      ? "Total"
      : subjectCount > 1
      ? "/month"
      : "/month per subject";

    priceWrapper.innerHTML = `<span class="price-value">${totalPrice.toFixed(
      2
    )}</span> <span class="price-term">${activeCurrency} ${paymentTerm}</span>`;
  ...
  async function init() {
    ...
    const defaultLevel = "IGCSE_GCSE";
    const firstLevelTab = document.querySelector(
      `.level-tab[data-level="${defaultLevel}"]`
    );

    // Set initial state for desktop view if applicable
    if (window.innerWidth >= 1200) {
      if (firstLevelTab) {
        handleLevelSelection(defaultLevel);
      }
    } else {
      // Set initial state for mobile view
      if (firstLevelTab) {
        handleLevelSelection(defaultLevel);
        // After populating courses for the level, select the first one
        const firstCourseItem = courseMenu.querySelector(
          ".custom-dropdown-item"
        );
        if (firstCourseItem) {
          const courseKey = firstCourseItem.dataset.value;
          const courseTitle = firstCourseItem.textContent;
          courseToggle.querySelector("span").textContent = courseTitle;
          courseToggle.dataset.value = courseKey;
          renderSingleCard(defaultLevel, courseKey);
        }
      }
    }
  }
