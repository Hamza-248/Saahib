document.addEventListener("DOMContentLoaded", async function () {
  const metadataUrl =
    "https://raw.githubusercontent.com/Hamza-248/Saahib/153eed46a5964d0d675835cb48ca3845ae645018/assets/data/blogs_metadata.JSON";
  const contentBaseUrl =
    "https://raw.githubusercontent.com/Hamza-248/Saahib/153eed46a5964d0d675835cb48ca3845ae645018/";
  ...
  const getCustomRenderer = () => {
    ...
    renderer.blockquote = (quote) => {
      ...
      if (noteRegex.test(quote))
        return `<blockquote class="alert alert-note">${quote.replace(
          noteRegex,
          "$1"
        )}</blockquote>`;
      if (warningRegex.test(quote))
        return `<blockquote class="alert alert-warning">${quote.replace(
          warningRegex,
          "$1"
        )}</blockquote>`;
      if (tipRegex.test(quote))
        return `<blockquote class="alert alert-tip">${quote.replace(
          tipRegex,
          "$1"
        )}</blockquote>`;
    ...
    renderer.image = (href, title, text) => {
      return `<figure><img src="${href}" alt="${text}" title="${title || ""}">${
    ...
    sidebarContainer.innerHTML = `
      ...
      post.author
                        ...
                        <p class="text-white-50 small">${
                          post.authorBio || "Author of this article."
                        }</p>
                    </div>
