            ...
            <div class="row gy-5 gx-lg-5">
                ...
                <div class="col-lg-2 col-md-4 col-6">
                    ...
                    <ul class="footer-links">
                        ...
                        <li><a href="index.html#Registration">Book a Trial</a></li>
                    </ul>
                </div>
...
function initializeHeaderScripts() {
  ...
  if (menuTrigger && nav) {
    ...
    console.error(
      "[Header] Could not find the menu trigger or nav element. Event listener not attached."
    );
  }
  ...
  // --- Sub-menu Logic ---
  const hasSubItems = headerContainer
    ? headerContainer.querySelectorAll(".has-sub")
    : [];
    ...
    item.addEventListener("click", function (event) {
      ...
      if (isMobile()) {
        ...
        if (subMenu) {
          ...
          headerContainer
            .querySelectorAll(".has-sub .sub-menu.show")
            .forEach((menu) => {
...
function initializeRegionLogic() {
  const regionOptions = document.querySelectorAll(
    ".header-area .region-option"
  );
  const currentRegionDisplay = document.getElementById(
  ...
  async function initializeRegion() {
    ...
    if (regionFromUrl && supportedRegions.includes(regionFromUrl)) {
      ...
      regionFromStorage &&
      supportedRegions.includes(regionFromStorage)
    ) {
      ...
      try {
        ...
        const finalCode = supportedRegions.includes(ipCountryCode)
          ? ipCountryCode
          : "sa";
...
function initializeGradient() {
  try {
    ...
    console.warn(
      "Gradient.js library not found or failed to initialize.",
      error
    );
  }
}
